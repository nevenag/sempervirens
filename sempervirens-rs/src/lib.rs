...
fn correct_pivot_subtree_columns(pivot_col: &Array1<f64>, mat: &Array2<f64>, _fpp: f64, _fnp: f64) -> Vec<Ix> {
...
fn noisy_pivot_subtree_columns(pivot_col: &ArrayBase<ViewRepr<&f64>, Ix1>, cols: &[Ix], mat: &Array2<f64>, fpp: f64, fnp: f64) -> Vec<Ix> {
    ...
    if fnp >= fpp {
        let c0 = f64::ln((1.0-fnp)/fpp);
        let c1 = f64::ln((1.0-fpp)/fnp);
...
fn reconstruct_root(cols_in_subtree: &[Ix], mat: &Array2<f64>, fnp: f64) -> Array1<f64> {
    ...
    azip!((out in &mut cols_mask, &count in &counts) *out = if count as f64 >= max_count*(1.0-fnp) { 1 } else { 0 });
...
fn column_max_likelihood_refinement(reconstructed_mat: &ArrayBase<ViewRepr<&f64>, Ix2>, noisy_mat: &ArrayBase<ViewRepr<&f64>, Ix2>, fpp: f64, fnp: f64, mep: f64) -> Array2<f64> {
    ...
    let log_probs = (1.0-fpp-mep).ln()*k_00s+ fnp.ln()*k_01s + fpp.ln()*k_10s + (1.0-fnp-mep).ln()*k_11s;
...
fn split_part(mat: &mut Array2<f64>, fpp: f64, fnp: f64) -> (Vec<Vec<usize>>, Array1<f64>, usize) {
    ...
    let (pivot_col_i, cols_in_subtree) = {
        ...
        while temp_part.len() > 0 {
            ...
            let candidate_cols_in_subtree = noisy_pivot_subtree_columns(&candidate_pivot, &temp_part, mat, fpp, fnp);
    ...
    let root = reconstruct_root(&cols_in_subtree, &mat, fnp);
    ...
    let final_cols_in_subtree = correct_pivot_subtree_columns(&reconstructed_pivot, &mat, fpp, fnp);
...
///     fnp: False negative probability.
///     mep: Missing entry probability.
/// 
/// Returns:
...
pub fn reconstruct(noisy: &Array2<f64>, fpp: f64, fnp: f64, mep: f64) -> Array2<f64> {
    assert!(
    ...
    while partition.len() > 0 {
        ...
        let (subpartitions, parts_columns): (Vec<Vec<Vec<usize>>>, Vec<(&Vec<usize>, Array2<f64>)>) = partition.par_iter()
            ...
            .map(|(part, mut columns)| {
                let (mut subpartition, reconstructed_pivot, col_i) = split_part(&mut columns, fpp, fnp);
    ...
    mat = column_max_likelihood_refinement(&mat.t(), &noisy.t(), fpp, fnp, mep).reversed_axes();
    mat = column_max_likelihood_refinement(&mat.view(), &noisy.view(), fpp, fnp, mep);
