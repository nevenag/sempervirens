...
def pick_pivot(prob_mat):
    ...
    for i in range(start_idx + 1, prob_mat.shape[1]):
        if expected_num_ones[i] != max_val:
            pass
...
def find_subtree(pivot, prob_mat):
...
def reconstruct_root(pivot, subtree, prob_mat):
...
def backward_pass(prob_mat, true_col, subtree):
    assert true_col.shape == (prob_mat.shape[0], 1)
    assert subtree.shape == (prob_mat.shape[1],)

    prob_mat = prob_mat*(1-subtree)*(1-true_col) + prob_mat*subtree*true_col
    return prob_mat
...
def reconstruct(measured, fpr, fnr):
    ...
    # Estimate prior over whole matrix
    ...
    while prob_mat.shape[1] > 1: # While more than 1 column remaining.
        ...
        subtree = find_subtree(pivot, prob_mat)

        # print(f'subtree after unions: \n{subtree}')
        reconstructed_root = reconstruct_root(pivot, subtree, prob_mat)
        # print(f'01d pivot: \n{reconstructed_root}')
        prob_mat = backward_pass(prob_mat, reconstructed_root, subtree)
